<template>
    <div class="m-block m-revision" id="revision">
        ...
        <div class="m-section">
            ...
            <span v-if="versions===false">⚠ 数据加载异常</span>
            <span v-if="versions&&!versions.length">No Versions</span>
            <table v-if="versions&&versions.length" id="histories">
                ...
                <tr class="history" v-for="(ver,key) in versions" :key='key'>
                    <td><a target="_blank" href="javascript:void(0)"
                           v-text="'v' + (versions.length - key)"></a></td>
    ...
    export default {
        ...
        methods: {
            get_versions() {
                let that = this;
                $.ajax({
                    url: `${JX3BOX.__helperUrl}api/achievement/${query.id}/versions`,
